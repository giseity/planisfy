...
write_smoke_env() {
  ...
  local override_re='^(DEPLOYMENT_MODE|STORAGE_PROVIDER|R2_ACCOUNT_ID|R2_BUCKET|R2_ENDPOINT|R2_PUBLIC_URL|R2_ACCESS_KEY_ID|R2_SECRET_ACCESS_KEY|AWS_ACCESS_KEY_ID|AWS_SECRET_ACCESS_KEY|DODO_PAYMENTS_API_KEY|DODO_PAYMENTS_API_URL|DODO_PAYMENTS_ENVIRONMENT|DODO_PAYMENTS_WEBHOOK_SECRET|DODO_STARTER_MONTHLY_PRODUCT_ID|DODO_STARTER_YEARLY_PRODUCT_ID|DODO_SCALE_MONTHLY_PRODUCT_ID|DODO_SCALE_YEARLY_PRODUCT_ID|DODO_PRO_PRODUCT_ID|DODO_ENTERPRISE_PRODUCT_ID|RESEND_API_KEY|FROM_EMAIL|LOCAL_STORAGE_HOST_PATH)='
  if [[ -f "$BASE_ENV_FILE" ]]; then
  ...
  write_env DODO_PAYMENTS_API_URL "${DODO_PAYMENTS_API_URL:-}"
  ...
  write_env DODO_STARTER_MONTHLY_PRODUCT_ID "${DODO_STARTER_MONTHLY_PRODUCT_ID:-${DODO_PRO_PRODUCT_ID:-managed-local-starter-monthly-product}}"
  write_env DODO_STARTER_YEARLY_PRODUCT_ID "${DODO_STARTER_YEARLY_PRODUCT_ID:-managed-local-starter-yearly-product}"
  write_env DODO_SCALE_MONTHLY_PRODUCT_ID "${DODO_SCALE_MONTHLY_PRODUCT_ID:-${DODO_ENTERPRISE_PRODUCT_ID:-managed-local-scale-monthly-product}}"
  write_env DODO_SCALE_YEARLY_PRODUCT_ID "${DODO_SCALE_YEARLY_PRODUCT_ID:-managed-local-scale-yearly-product}"
  write_env DODO_PRO_PRODUCT_ID "${DODO_PRO_PRODUCT_ID:-managed-local-pro-product}"
  write_env DODO_ENTERPRISE_PRODUCT_ID "${DODO_ENTERPRISE_PRODUCT_ID:-managed-local-enterprise-product}"
  write_env RESEND_API_KEY "${RESEND_API_KEY:-managed-local-resend-key}"
  write_env FROM_EMAIL "${FROM_EMAIL:-Planisfy <noreply@planisfy.com>}"
