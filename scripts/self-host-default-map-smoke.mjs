...
const martinUrl = stripTrailingSlash(process.env.MARTIN_INTERNAL_URL ?? 'http://localhost:3005')
...
if (tileBytes.byteLength < minTileBytes) {
  throw new Error(`Default map tile was too small: ${tileBytes.byteLength} bytes from ${tileUrl}`)
}
