...
OUTPUT_DIR="${OUTPUT_DIR:-$ROOT_DIR/dogfood-output/self-host-restart-persistence}"
API_URL="${PLANISFY_E2E_API_URL:-http://localhost:4000}"
CONSOLE_URL="${PLANISFY_E2E_CONSOLE_URL:-http://localhost:3001}"
SUFFIX="${PLANISFY_E2E_RESOURCE_SUFFIX:-restart-persistence-$(date -u +%Y%m%d%H%M%S)}"
...
cleanup() {
  ...
  if [[ "$status" -ne 0 ]]; then
    mkdir -p "$LOG_DIR"
    compose logs --no-color postgres redis minio minio-init api console worker-geodata worker-billing >"$LOG_DIR/compose.log" 2>&1 || true
  fi
...
echo "Preparing clean self-host restart-persistence stack"
compose down -v --remove-orphans >/dev/null 2>&1 || true
if [[ "$(storage_provider)" == "s3" ]]; then
...
compose up -d api console worker-geodata worker-billing
...
echo "Restarting core services without resetting volumes"
compose restart postgres redis worker-geodata worker-billing api console

wait_for_http "$API_URL/health" "Restarted API"
wait_for_http "$CONSOLE_URL" "Restarted Console"
...
echo "Checking authorized setup preflight after restart"
curl -fsS -H "x-internal-secret: $secret" "$API_URL/setup/preflight" >/dev/null

echo "Checking persisted public style after restart"
curl -fsS "$STYLE_URL" >/dev/null

echo "Checking persisted TileJSON after restart"
...
if (!Array.isArray(body.tiles) || body.tiles.length === 0) {
  console.error("Restarted TileJSON does not expose tile URL templates");
  process.exit(1);
}
if (!Array.isArray(body.vector_layers) || body.vector_layers.length === 0) {
  console.error("Restarted TileJSON does not expose vector layer metadata");
  process.exit(1);
}
NODE

echo "Self-host restart-persistence smoke passed"
