...
description: Register polling root agents for routing graph builds.
---

# External Compute

Planisfy uses polling root agents for external compute. A root agent is a server process that connects outbound to the API, registers as a worker node, polls for work, runs build tooling such as Valhalla, uploads artifacts, and finalizes metadata.

Polling agents are the default because they work behind NAT, firewalls, and VPS private networks. Planisfy does not require inbound webhook endpoints on worker machines for routing builds.
...
ROOT_AGENT_CAPABILITIES=valhalla_graph_build,dem_hydration,self_host_activation
ROOT_AGENT_POLL_INTERVAL_MS=5000
ROOT_AGENT_UPLOAD_RETRIES=5
ROOT_AGENT_UPLOAD_CONCURRENCY=4
```

Optional activation settings let the agent install completed artifacts into a local Compose stack and restart affected services:
...
ROOT_AGENT_COMPOSE_FILE=/opt/planisfy/infra/docker/docker-compose.yml
ROOT_AGENT_COMPOSE_ENV_FILE=/opt/planisfy/.env
ROOT_AGENT_COMPOSE_CWD=/opt/planisfy
```
