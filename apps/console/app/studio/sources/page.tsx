  ...
  const fetchTilesets = useCallback(async () => {
    try {
      const [tilesetsRes, jobsRes, importsRes, stylesRes, targetsRes, profilesRes] = await Promise.all([
        ...
        api.get<ApiEnvelope<StyleSummary[]>>("/styles"),
...
function SourceWorkflowGuide({
  ...
  const guide = buildWorkflowGuide(tilesets, sourceImports, styles);
    ...
    case "publish-tileset": {
      ...
      return (
        <Button
          ...
          disabled={
            publishingVersionId === publishTilesetAction.tileset.latestVersion?.id
          }
        >
          {publishingVersionId ===
...
function buildWorkflowGuide(
  ...
  let nextAction: WorkflowNextAction = { kind: "open-dashboard" };
  if (!hasSourceData) nextAction = { kind: "add-source" };
  else if (readyImport) {
  ...
  else if (hasPublishedTileset && !hasStyle) nextAction = { kind: "create-style" };
  else if (hasStyle && !hasPublicStyle) nextAction = { kind: "publish-style" };

  const steps: WorkflowStep[] = [
    {
      label: "Source data",
      detail: hasSourceData
        ? `${sourceImports.length + tilesets.length} item${
            sourceImports.length + tilesets.length === 1 ? "" : "s"
          }`
        : "Waiting",
    ...
    },
    {
      ...
      detail: hasPublishedTileset ? "TileJSON ready" : publishableTileset ? "Version ready" : "Pending build",
      state: hasPublishedTileset ? "done" : publishableTileset ? "current" : "pending",
    },
    {
      label: "Design style",
      detail: hasStyle
    ...
    {
      label: "Publish style",
      detail: hasPublicStyle ? "Integration ready" : hasStyle ? "Draft" : "Pending style",
