...
export default function StyleEditorPage() {
  ...
  if (!style) {
    return (
      <LoadingState className="h-screen rounded-none border-0" label="Loading style editor..." />
    )
  }
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden">
      ...
      <header className="flex h-10 items-center gap-2 border-b bg-background px-3">
        <Button asChild variant="ghost" size="icon" className="h-7 w-7" title="Back to styles">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Keep editing in draft, publish when ready, or copy a version-pinned link.
                    </p>
                  </div>
                  <Button
                    size="sm"
      ...
      >
        ...
        <ResizablePanel id="style-editor-map" defaultSize="56%" minSize="20rem" className="min-w-0">
          <main className="flex h-full min-w-0 flex-col">
            {showJson || showValidation ? (
              <ResizablePanelGroup
                id="style-editor-center-panels"
                orientation="vertical"
                defaultLayout={{
                  'style-editor-map-preview': 58,
                  'style-editor-lower-panels': 42,
                }}
                className="min-h-0 flex-1"
              >
                <ResizablePanel
                  id="style-editor-map-preview"
                  defaultSize="58%"
                  minSize="14rem"
                  className="min-h-0"
                >
                ...
                >
                  <div className="flex h-full min-w-0">
                    {showJson && (
                      <div
                        className={showValidation ? 'min-w-0 flex-1 border-r' : 'min-w-0 flex-1'}
                      >
                        <JsonEditor />
                      </div>
                    )}
                    {showValidation && (
                      <div className={showJson ? 'w-72 shrink-0' : 'min-w-0 flex-1'}>
                        <ValidationPanel />
                      </div>
                    )}
                  </div>
