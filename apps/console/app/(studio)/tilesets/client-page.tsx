...
export default function SourcesPage() {
  ...
  const [rebuildingTilesetId, setRebuildingTilesetId] = useState<string | null>(null)
  const [deleteTileset, setDeleteTileset] = useState<ConsoleTileset | null>(null)
  const [deletingTilesetId, setDeletingTilesetId] = useState<string | null>(null)

  const fetchTilesets = useCallback(async () => {
    try {
      const [tilesetsRes, jobsRes] = await Promise.all([api.listTilesets(), api.listJobs()])
  ...
  async function handlePublish(tileset: ConsoleTileset, version = tileset.latestVersion) {
    if (!version) return
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Failed to promote tileset version')
    } finally {
  ...
  async function handleRebuild(tileset: ConsoleTileset) {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Failed to rebuild tileset')
    } finally {
  ...
  async function handleCancelJob(job: ConsoleProcessingJob) {
    ...
    try {
      ...
      toast.success(job.status === 'PENDING' ? 'Queued job canceled' : 'Cancellation requested')
      fetchTilesets()
    } catch (err) {
  ...
  return (
    <div className="py-8">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="mb-4 mt-1 text-sm text-muted-foreground">
            Create a tileset, then attach an upload or Overture source from its detail page.
          </p>
          <div className="flex items-center gap-2">
            <Button onClick={() => setCreateOpen(true)} data-testid="create-tileset-empty">
              <Plus className="mr-2 h-4 w-4" />
              Create tileset
            </Button>
          </div>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete &ldquo;{deleteTileset?.name}&rdquo;?</AlertDialogTitle>
            <AlertDialogDescription>
              This removes the tileset from Console and makes its public TileJSON unavailable.
              Uploaded source files and processed artifacts remain in storage for audit and backup
              retention.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={Boolean(deletingTilesetId)}>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleDeleteTileset} disabled={Boolean(deletingTilesetId)}>
