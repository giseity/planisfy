...
import { AlertCircle, Database, Globe, Loader2, MoreHorizontal, Upload } from 'lucide-react'
      ...
      <div className="flex min-h-[360px] items-center justify-center rounded-lg border">
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            No tileset matched this ID or handle.
          </p>
        </div>
      </div>
    )
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <div className="mb-1 flex items-center gap-2 text-sm text-muted-foreground">
            <Link href="/tilesets" className="hover:text-foreground">
              Tilesets
            </Link>
            <span>/</span>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
              <Fact
                label="Total size"
                value={formatBytes(
                  currentVersion?.artifact?.size ?? tileset.latestUpload?.size ?? null
                )}
              />
              <Fact label="Layers" value={layerCount(currentVersion, tileset).toString()} />
              <Fact
                label="Zoom range"
                value={`${tileset.minZoom ?? currentVersion?.minZoom ?? 0}-${tileset.maxZoom ?? currentVersion?.maxZoom ?? 14}`}
              />
              <Fact
                label="Owner"
                value={tileset.ownerHandle ? `@${tileset.ownerHandle}` : 'Organization'}
              />
          ...
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              <Badge variant={hasSource ? statusVariant(tileset.status) : 'secondary'}>
                {!hasSource && tileset.status === 'DRAFT' ? 'Awaiting source' : tileset.status}
              </Badge>
              {tileset.latestUpload && (
                <Badge variant="outline">Upload: {tileset.latestUpload.status}</Badge>
              )}
              {tileset.latestSourceImport && (
                <Badge variant="outline">Overture: {tileset.latestSourceImport.status}</Badge>
              )}
            </div>
            <p className="text-sm text-muted-foreground">{sourceSetupMessage(tileset)}</p>
                    ...
                    versions.map((version) => (
                      <TableRow key={version.id}>
                        ...
                        <TableCell className="font-mono text-xs text-muted-foreground">
                          {formatBytes(version.artifact?.size ?? null)}
                        </TableCell>
                        <TableCell className="font-mono text-xs text-muted-foreground">
                          {version.minZoom ?? 0}-{version.maxZoom ?? 14}
                        </TableCell>
                        <TableCell>
                          <Button
                            variant="ghost"
        ...
        <TabsContent value="metadata">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Origin and build configuration for the active tileset.
              </CardDescription>
            </CardHeader>
            <CardContent className="grid gap-3 sm:grid-cols-2">
              ...
              <MetadataBlock
                ...
                detail={
                  currentVersion ? `format ${currentVersion.format}` : 'No artifact published'
                }
              />
            </CardContent>
          </Card>
...
function featureCountDetail(value?: number | null) {
  return typeof value === 'number' ? `${formatNumber(value)} features` : 'Feature count unavailable'
}
