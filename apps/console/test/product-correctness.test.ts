...
describe('console product correctness regressions', () => {
  ...
  it('handles verification resend API errors', () => {
    const source = readFileSync(
      resolve(__dirname, '../components/shell/email-verification-banner.tsx'),
      'utf8'
    )
  ...
  it('syncs email verification status from the profile API', () => {
    ...
    expect(verifyPage).toContain('api.getProfile()')
    expect(verifyPage).toContain('router.replace(callbackUrl)')
    expect(banner).toContain('api.getProfile()')
    expect(banner).toContain('setFreshEmailVerified')
  })

  it('sends welcome email for verified OAuth-created users', () => {
    const source = readFileSync(
      resolve(__dirname, '../../../packages/auth/src/server/auth.ts'),
      'utf8'
    )

    expect(source).toContain('/internal/send-welcome-email')
    expect(source).toContain('if (userData.emailVerified)')
    expect(source).toContain('await sendWelcomeEmail(userData)')
  })

  it('keeps platform readiness out of managed console navigation', () => {
    const source = readFileSync(resolve(__dirname, '../lib/console-navigation.tsx'), 'utf8')
  ...
  it('keeps platform-maintenance operation tabs self-host only', () => {
    const source = readFileSync(resolve(__dirname, '../features/operations/provider.tsx'), 'utf8')
  ...
  it('does not expose stale job reconciliation from console jobs', () => {
    const source = readFileSync(resolve(__dirname, '../features/operations/jobs-tab.tsx'), 'utf8')
  ...
  it('gates onboarding readiness outside managed console access', () => {
    const source = readFileSync(resolve(__dirname, '../app/onboarding/layout.tsx'), 'utf8')
  ...
  it('refetches sprite metadata when the sprite URL changes', () => {
    const source = readFileSync(
      resolve(__dirname, '../features/style-editor/components/fields/icon-field.tsx'),
      'utf8'
    )
  ...
  it('does not visually parse expression-style filters', () => {
    ...
    expect(canParseFilter(['in', 'class', ['literal', ['park']]])).toBe(false)
  })
