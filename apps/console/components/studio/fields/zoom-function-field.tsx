...
export function ZoomFunctionField({
  ...
  const updateStop = (index: number, field: "zoom" | "value", newVal: unknown) => {
  ...
  const addStop = () => {
    ...
    const lastValue = stops.length > 0 ? stops[stops.length - 1]!.value : getDefault(spec)
    onChange({
      ...value,
      ...
      {stops.map((stop, i) => (
        <div key={i} className="flex items-center gap-1">
          ...
          <Input
            type="number"
            value={stop.zoom}
            onChange={(e) => updateStop(i, "zoom", parseFloat(e.target.value) || 0)}
 ...
 */
export function valueToZoomFunction(value: unknown, spec: PropertySpec): ZoomFunction {
