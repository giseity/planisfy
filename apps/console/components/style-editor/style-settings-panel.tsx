      ...
      <div className="flex flex-col gap-3 p-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <NumberField
            ...
            value={center?.[0] ?? 0}
            onChange={(v) =>
              updateStyleTopLevel("center", [v, center?.[1] ?? 0])
            }
            min={-180}
            max={180}
            step={0.01}
          ...
          <NumberField
            ...
            value={center?.[1] ?? 0}
            onChange={(v) =>
              updateStyleTopLevel("center", [center?.[0] ?? 0, v])
            }
            min={-90}
            max={90}
            step={0.01}
        ...
        {mapPosition && (
          <div className="rounded bg-muted/30 p-2 text-[10px] text-muted-foreground font-mono">
            <div>
              Current: {mapPosition.center[0].toFixed(4)},{" "}
              {mapPosition.center[1].toFixed(4)}
            </div>
            <div>
              Zoom: {mapPosition.zoom.toFixed(2)} | Bearing:{" "}
              {mapPosition.bearing.toFixed(0)} | Pitch:{" "}
              {mapPosition.pitch.toFixed(0)}
            </div>
          </div>
        )}
      </div>
