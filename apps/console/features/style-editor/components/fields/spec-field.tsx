...
import { getValueType } from "@/features/style-editor/style-spec";
...
export function SpecField({
  ...
  if (
    ...
    return (
      <FontField
        label={property}
        value={
          Array.isArray(value) ? value : ((spec.default as string[]) ?? [])
        }
        onChange={onChange}
        glyphsUrl={glyphsUrl}
      />
  ...
  if (
    (property === "icon-image" || property.endsWith("-pattern")) &&
    valueType === "value"
  ) {
    return (
      <IconField
  ...
  if (valueType === "expression") {
    return (
      <ExpressionField label={property} value={value} onChange={onChange} />
    );
  }
    ...
    case "number":
      return (
        <NumberField
          label={property}
          value={
            typeof value === "number" ? value : ((spec.default as number) ?? 0)
          }
          onChange={onChange}
          min={spec.minimum ?? 0}
          ...
          showSlider={
            spec.maximum !== undefined || property.includes("opacity")
          }
        />
      );
      ...
      return (
        <EnumField
          ...
          value={
            typeof value === "string"
              ? value
              : ((spec.default as string) ?? options[0] ?? "")
          }
          options={options}
          onChange={onChange}
        />
    ...
    case "array":
      return (
        <ArrayField
          label={property}
          value={
            Array.isArray(value) ? value : ((spec.default as unknown[]) ?? [])
          }
          onChange={onChange}
    ...
    case "formatted":
      return (
        <StringField
          label={property}
          value={
            typeof value === "string" ? value : ((spec.default as string) ?? "")
          }
