...
import { useStyleStore, type SourceLayerOptions } from '@/features/style-editor/store/style-store'
    ...
    <ScrollArea className="h-full">
      <div className="flex flex-col gap-3 p-3">
        <SourceBrowser
          ...
          onAddLayer={(sourceId, options) => addLayerFromSource(sourceId, options)}
        />
...
function SourceBrowser({
  ...
  const [selectedSourceLayers, setSelectedSourceLayers] = useState<Record<string, string>>({})
  const [selectedPlatformLayers, setSelectedPlatformLayers] = useState<Record<string, string>>({})
  const platformSources = isManagedDeploymentMode(clientEnv.NEXT_PUBLIC_DEPLOYMENT_MODE)
    ...
    <div className="rounded border bg-muted/20 p-2 space-y-3">
      ...
      {platformSources.length > 0 && (
        <div className="space-y-1">
          ...
          {platformSources.map((source) => {
            ...
            return (
              <div key={source.id} className="rounded border bg-background p-2 space-y-1">
      ...
      {error ? (
        ...
        <div className="space-y-1">
          ...
          <p className="text-[11px] text-muted-foreground">No uploaded tilesets yet.</p>
        </div>
      ) : (
        <div className="space-y-1">
          <p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">
          ...
          {tilesets.slice(0, 8).map((tileset) => {
            ...
            const selectedSourceLayer = selectedSourceLayers[tileset.id] ?? vectorLayers[0]?.id
            return (
              <div key={tileset.id} className="rounded border bg-background p-2 space-y-1">
                <div className="flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    <div className="truncate text-xs font-medium">{tileset.name}</div>
                    <div className="truncate font-mono text-[10px] text-muted-foreground">
                      {tileset.ownerHandle
...
function SourceItem({
  ...
  const [layerType, setLayerType] = useState<NonNullable<SourceLayerOptions['layerType']>>(
  ...
  return (
    <div className="rounded border p-2 space-y-2">
      ...
      <div className="rounded bg-muted/30 p-1.5 space-y-1">
        <Label className="text-[10px] text-muted-foreground">Create layer</Label>
        <div className="flex gap-1">
          <Select
            value={layerType}
            onValueChange={(v) => setLayerType(v as NonNullable<SourceLayerOptions['layerType']>)}
          >
            <SelectTrigger className="!h-6 flex-1 rounded-md py-0 pl-2 pr-1 text-xs [&_svg]:size-3">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {layerTypesForSource(source).map((type) => (
                <SelectItem key={type} value={type} className="!min-h-6 py-0.5 text-xs">
                  {type}
                </SelectItem>
              ))}
            </SelectContent>
          ...
          <Button
            ...
            onClick={() => onAddLayer({ layerType, sourceLayer: sourceLayer || undefined })}
          >
