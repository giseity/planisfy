...
import {
  ...
  type PropertySpec,
} from "@/features/style-editor/style-spec";
...
export function PropertyPanel() {
  ...
  const paint = ("paint" in layer ? layer.paint : {}) as
    | Record<string, unknown>
    | undefined;
  const layout = ("layout" in layer ? layer.layout : {}) as
    | Record<string, unknown>
    | undefined;
  ...
  return (
    <ScrollArea className="h-full">
      <div className="flex flex-col gap-3 p-3">
        ...
        <LayerHeader
          layer={layer}
          onTopLevelChange={updateLayerTopLevel}
          style={style}
        />
        ...
        {"filter" in layer && (
          <>
            ...
            <FilterField
              value={layer.filter}
              onChange={(v) =>
                updateLayerTopLevel(selectedLayerId, "filter", v)
              }
            />
          </>
        )}
