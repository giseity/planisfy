...
import { areaOfInterestToBBox, areaOfInterestToHgtTileNames } from '@planisfy/api-contracts'
import {
  Activity,
  CheckCircle2,
  Copy,
...
export function RoutingTab({
  ...
  const [detail, setDetail] = useState<ConsoleRoutingGraphBuildDetail | null>(null)
  ...
  async function loadDetail(id: string) {
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Failed to load routing build')
    }
  }
      ...
      <div className="space-y-4">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Create a one-time token for a server running the root-agent service.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            {registrationToken ? (
              <div className="rounded-md border p-3 text-sm">
                ...
                <div className="break-all font-mono text-xs">{registrationToken}</div>
                <Button
                  className="mt-3"
                  size="sm"
                  variant="outline"
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Build a Valhalla graph on a registered root agent.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            <label className="flex items-center gap-2 text-sm">
              <Checkbox
                ...
                onCheckedChange={(value) => setIncludeAdmins(Boolean(value))}
              />
              Include admin database
            </label>
            <label className="flex items-center gap-2 text-sm">
              <Checkbox
                checked={includeTimezones}
                onCheckedChange={(value) => setIncludeTimezones(Boolean(value))}
              />
            ...
            <Field label="Elevation">
              <Select
                value={elevationMode}
                onValueChange={(value) => setElevationMode(value as 'none' | 'dem_companion')}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
