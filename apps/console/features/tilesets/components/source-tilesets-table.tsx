...
export function SourceTilesetsTable({
  ...
  onPublish: (tileset: ConsoleTileset, version?: ConsoleTilesetVersion | null) => void
    ...
    <Table>
      ...
      <TableBody>
        {tilesets.map((tileset) => {
          const displayVersion = tileset.currentVersion ?? tileset.latestVersion
                ...
                <TableCell>
                  ...
                  {latestJob && (
                    <div className="mt-1 text-xs text-muted-foreground">
                      ...
                      {latestJob.status !== 'SUCCEEDED' ? ` - ${latestJob.progress}%` : ''}
                    </div>
                  )}
                </TableCell>
                ...
                <TableCell className="w-10 text-right">
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button variant="ghost" size="icon-sm" aria-label={`${tileset.name} actions`}>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem
                        ...
                        disabled={!canPublish || publishingVersionId === tileset.latestVersion?.id}
                        onSelect={() => onPublish(tileset)}
                      >
                      ...
                      <DropdownMenuItem
                        disabled={!canRebuildTileset(tileset) || rebuildingTilesetId === tileset.id}
                        onSelect={() => onRebuild(tileset)}
                      >
                      ...
                      <DropdownMenuItem
                        ...
                        onSelect={() =>
                          tileset.tilejsonUrl &&
                          window.open(tileset.tilejsonUrl, '_blank', 'noopener,noreferrer')
                        }
                      >
                      ...
                      {latestJob && canCancelJob(latestJob) && (
                        <DropdownMenuItem
                          disabled={controllingJobId === latestJob.id}
                          onSelect={() => onCancelJob(latestJob)}
                        >
                          {controllingJobId === latestJob.id ? (
                            <RefreshCw className="mr-2 h-4 w-4 animate-spin" />
                          ) : (
                      ...
                      <DropdownMenuItem
                        ...
                        onSelect={() => onRequestDelete(tileset)}
                      >
                  ...
                  <TableCell colSpan={8} className="bg-muted/20">
                    <div className="space-y-2 py-2 text-xs text-muted-foreground">
                      ...
                      <div className="grid gap-2 md:grid-cols-3">
                        ...
                        <div>
                          <div className="font-medium text-foreground">Artifact</div>
                      ...
                      {tileset.versions.length > 1 && (
                        <div className="flex flex-wrap items-center gap-1 border-t pt-2">
                          <span className="mr-1 font-medium text-foreground">Versions</span>
                          {tileset.versions.map((version) => {
                            const current = version.id === tileset.currentVersionId
                            const currentVersionNumber =
                              tileset.currentVersion?.version ??
                              tileset.versions.find(
                                (candidate) => candidate.id === tileset.currentVersionId
                              )?.version
                            return (
                              <Button
                                key={version.id}
...
function boundsSummary(bounds: [number, number, number, number] | null | undefined) {
...
function artifactSummary(version: ConsoleTilesetVersion | null) {
  ...
  const size = version.artifact.size ? formatBytes(version.artifact.size) : 'unknown size'
...
function latestJobForTileset(
  ...
  return (
    jobs.find(
      (job) =>
        (job.type === 'tileset.process_upload' || job.type === 'tileset.process_dataset') &&
...
function canCancelJob(job: ConsoleProcessingJob) {
  return (job.status === 'PENDING' || job.status === 'PROCESSING') && !job.cancelRequestedAt
}
