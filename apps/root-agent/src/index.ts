...
const envSchema = z.object({
  ...
  ROOT_AGENT_CAPABILITIES: z.string().default('valhalla_graph_build,basemap_build,dem_hydration'),
  ...
  ROOT_AGENT_VALHALLA_DATA_DIR: z.string().default('/opt/planisfy/infra/docker/data/valhalla_data'),
  ROOT_AGENT_ELEVATION_DATA_DIR: z.string().default('/opt/planisfy/infra/docker/data/elevation'),
  ...
  ROOT_AGENT_ACTIVATION_WORK_TTL_HOURS: z.coerce.number().int().min(1).default(24),
})
...
type BasemapRuntimeTarget = {
  releaseId: string
  artifactId: string
  martinSource: string
  martinSourceVersioned: string
  martinPrimarySource: string
...
async function main() {
  ...
  for (;;) {
    try {
      if (Date.now() - lastActivationCleanup > 60 * 60 * 1000) {
        await cleanupStaleActivationWork()
        lastActivationCleanup = Date.now()
      }
  ...
  try {
    ...
    if (build.includeTimezones) {
      await updateBuild(
        ...
        'building_admins',
        32,
        'Building Valhalla timezone database'
      )
...
async function buildBasemap(token: string, build: BasemapBuild) {
  ...
  try {
    ...
    await updateBuild(
      ...
      'building_tiles',
      35,
      'Building basemap tiles with Planetiler'
    )
    ...
    await uploadArtifact(token, build, outputPath, {
      ...
      checksumSha256,
      contentType:
        extension === 'pmtiles' ? 'application/vnd.pmtiles' : 'application/vnd.mapbox-vector-tile',
      manifest: {
        buildId: build.id,
        sourceUrl: build.sourceUrl,
        sourcePreset: build.sourcePreset,
...
async function buildDemCompanion(token: string, build: RoutingGraphBuild, buildDir: string) {
  ...
  if (!tileNames.length) {
    await sendLogs(token, build.id, [
      {
        level: 'warn',
        message: 'DEM companion requested but no DEM bounds or HGT tile names were configured.',
      },
    ])
    return
  }
...
async function activateRoutingGraph(
  ...
  try {
    ...
    await restartRuntimeServices(token, build.id, ['valhalla', ...(dem ? ['elevation'] : [])])
...
async function activateBasemap(
  ...
  if (!runtimeTarget) {
    await updateActivation(token, build.id, 'failed', 'No basemap runtime target was provided')
    return
  }
  const target = validateBasemapRuntimeTarget(runtimeTarget)
  const activationDir = join(config.ROOT_AGENT_WORK_DIR, 'activation', build.id, 'basemap')
  const incomingDir = join(config.ROOT_AGENT_MARTIN_SOURCES_DIR, '.incoming')
  const artifactPath = join(incomingDir, `${target.artifactId}.${target.extension}.tmp`)
  const stableSource = target.martinSource
  const versionedSource = target.martinSourceVersioned
  const stablePath = join(
    config.ROOT_AGENT_MARTIN_SOURCES_DIR,
    `${stableSource}.${target.extension}`
  )
  const versionedPath = join(
    config.ROOT_AGENT_MARTIN_SOURCES_DIR,
    `${versionedSource}.${target.extension}`
  )
  const staleExtension = target.extension === 'pmtiles' ? 'mbtiles' : 'pmtiles'
  try {
    await mkdir(config.ROOT_AGENT_MARTIN_SOURCES_DIR, { recursive: true })
    await mkdir(incomingDir, { recursive: true })
    await rm(activationDir, { recursive: true, force: true })
    await rm(join(config.ROOT_AGENT_MARTIN_SOURCES_DIR, `${stableSource}.${staleExtension}`), {
      force: true,
    })
    await rm(join(config.ROOT_AGENT_MARTIN_SOURCES_DIR, `${versionedSource}.${staleExtension}`), {
      force: true,
    })
    ...
    await rename(artifactPath, versionedPath)
    ...
    await updateActivation(token, build.id, 'active', 'Basemap artifact activated', {
      output: {
        ...
        martinPrimarySource: target.martinPrimarySource,
        martinSourcesDir: config.ROOT_AGENT_MARTIN_SOURCES_DIR,
...
async function restartRuntimeServices(token: string, buildId: string, services: string[]) {
...
async function uploadArtifactViaApi(
  ...
  await withRetry(async () => {
    ...
    const response = await fetch(`${apiBase}/root-agent/jobs/${build.id}/artifacts`, uploadInit)
...
async function uploadMultipartArtifact(
  ...
  await Promise.all(
    Array.from({ length: Math.min(config.ROOT_AGENT_UPLOAD_CONCURRENCY, parts.length) }, () =>
      uploadNextPart()
    )
  )
...
async function linkFileAtomic(from: string, to: string) {
  ...
  try {
    ...
    await rm(tmp, { force: true })
    throw new Error(
      `Martin stable alias hardlink failed; ensure ROOT_AGENT_MARTIN_SOURCES_DIR is on one filesystem that supports hardlinks. ${
        err instanceof Error ? err.message : String(err)
      }`
    )
  }
...
async function cleanupStaleActivationWork() {
  const ttlMs = config.ROOT_AGENT_ACTIVATION_WORK_TTL_HOURS * 60 * 60 * 1000
  const cutoff = Date.now() - ttlMs
  await cleanupStaleChildren(join(config.ROOT_AGENT_WORK_DIR, 'activation'), cutoff)
  await cleanupStaleChildren(join(config.ROOT_AGENT_MARTIN_SOURCES_DIR, '.incoming'), cutoff)
  await cleanupStaleChildren(
    join(dirname(config.ROOT_AGENT_VALHALLA_DATA_DIR), '.incoming'),
    cutoff
  )
}

async function cleanupStaleChildren(root: string, cutoff: number) {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      console.log(`[root-agent] pruned stale activation path ${path}`)
    } catch (err) {
      console.warn(
        `[root-agent] stale activation cleanup failed for ${path}: ${
          err instanceof Error ? err.message : String(err)
        }`
      )
    }
...
function validateBasemapRuntimeTarget(target: BasemapRuntimeTarget) {
  assertSafeMartinSource(target.martinSource, 'martinSource')
  assertSafeMartinSource(target.martinSourceVersioned, 'martinSourceVersioned')
  assertSafeMartinSource(target.martinPrimarySource, 'martinPrimarySource')
  if (target.extension !== 'pmtiles' && target.extension !== 'mbtiles') {
    throw new Error('Basemap runtime target extension must be pmtiles or mbtiles')
  }
  return target
}

function assertSafeMartinSource(value: string, label: string) {
  if (!/^[A-Za-z0-9._-]{1,256}$/.test(value)) {
    throw new Error(`Unsafe basemap runtime target ${label}: ${value}`)
  }
}
...
function parseDemConfig(value: unknown): DemConfig {
  ...
  return {
    bounds:
      bounds && [bounds.minLon, bounds.minLat, bounds.maxLon, bounds.maxLat].every(Number.isFinite)
        ? bounds
        : undefined,
