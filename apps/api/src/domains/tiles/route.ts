...
tilesRoute.get('/tiles/v1/:owner/:handle.json', async (c) => {
  ...
  const resolved = await resolveTileset(parsed.owner, parsed.handle, parsed.version)
  if (!resolved?.version) {
    return c.json(
      {
        error: {
          code: 'NOT_FOUND',
          message: parsed.version ? 'Tileset version not found' : 'Tileset not found',
        },
      },
      404
    )
  ...
  return c.json(toTileJson(resolved, parsed.version ? 'version' : 'stable'), 200, {
    'Cache-Control': parsed.version ? 'public, max-age=31536000, immutable' : 'public, max-age=300',
...
tilesRoute.get('/tiles/v1/:source{.+\\.json$}', async (c) => {
  ...
  if (parsed) {
    ...
    return c.json(toTileJson(resolved, parsed.version ? 'dotted-version' : 'dotted-stable'), 200, {
...
tilesRoute.get('/v4/:tileset/tilequery/:coords.json', async (c) => {
  ...
  const resolved = await resolveTileset(tileset.owner, tileset.handle, tileset.version)
  if (!resolved?.version) {
    return c.json({ error: { code: 'NOT_FOUND', message: 'Tileset not found' } }, 404)
  }
...
async function legacyMartinTileJson(c: ApiContext, source: string) {
  const martinUrl = buildMartinSourceUrl(env.MARTIN_INTERNAL_URL, source)
  if (!martinUrl) {
    return c.json({ error: { code: 'VALIDATION_ERROR', message: 'Invalid tile source' } }, 400)
  }
  ...
  try {
    ...
    tileJson.tiles = [`${apiBase}/tiles/v1/${encodeURIComponent(source)}/{z}/{x}/{y}`]
...
async function proxyMartinTile(c: ApiContext, path: string) {
  const martinUrl = buildMartinTileUrl(env.MARTIN_INTERNAL_URL, path)
  if (!martinUrl) {
    return c.json(
      {
        error: {
...
export function martinTileResponseHeaders(upstreamHeaders: Headers) {
  return {
    contentType: upstreamHeaders.get('content-type') || 'application/x-protobuf',
...
export function buildMartinSourceUrl(baseUrl: string, source: string) {
  ...
  return new URL(encodeURIComponent(source), `${baseUrl.replace(/\/$/, '')}/`).toString()
}
...
async function proxyTileWorker(c: ApiContext, pathWithQuery: string) {
  const tileWorkerUrl = tileWorkerUrlForPath(env.TILE_WORKER_INTERNAL_URL, pathWithQuery)
  if (!tileWorkerUrl) {
    return c.json(
      {
        error: {
          code: 'TILE_WORKER_UNCONFIGURED',
          message: 'Tile worker delivery mode requires TILE_WORKER_INTERNAL_URL.',
        },
      },
      503
    )
