...
const GLYPHS_BASE_URL = env.GLYPHS_INTERNAL_URL
...
fontsRoute.get('/fonts/v1/:fontstack/:range', async (c) => {
  ...
  const upstreamUrl = glyphRange ? buildGlyphUrl(GLYPHS_BASE_URL, fontstack, glyphRange) : null

  if (!upstreamUrl) {
    return c.json({ error: { code: 'VALIDATION_ERROR', message: 'Invalid font range' } }, 400)
  }
  ...
  try {
    ...
    return c.json({ error: { code: 'INTERNAL_ERROR', message: 'Font server unavailable' } }, 503)
  }
})
...
export function buildGlyphUrl(baseUrl: string, fontstack: string, glyphRange: string) {
