...
const STATIC_BASE_URL = env.STATIC_RENDERER_INTERNAL_URL
...
staticMapRoute.get('/static/v1/:owner/:style/:center/:size{.+\\.png$}', async (c) => {
  ...
  if (isNaN(width) || isNaN(height) || width < 1 || height < 1) {
    return c.json(
      { error: { code: 'BAD_REQUEST', message: 'Size format: widthxheight (e.g. 800x600)' } },
      400
    )
  }
  ...
  if (STATIC_BASE_URL) {
    try {
      ...
      if (!res.ok) {
        return c.json(
          { error: { code: 'UPSTREAM_ERROR', message: 'Static map render failed' } },
          502
        )
      }
      ...
      return c.json(
        { error: { code: 'INTERNAL_ERROR', message: 'Static map service unavailable' } },
        503
      )
    }
  }
