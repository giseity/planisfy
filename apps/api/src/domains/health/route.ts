...
healthRoute.get('/metrics', (c) => {
  if (!isDiagnosticsRequestAllowed(c.req.raw.headers)) {
    return c.json({ error: { code: 'UNAUTHORIZED', message: 'Diagnostics access denied' } }, 401)
  }

  return c.text(renderPrometheusMetrics({ service: 'api', version: env.APP_VERSION }), 200, {
...
healthRoute.get('/health/detailed', async (c) => {
  ...
  // Martin (tile server)
  const martinUrl = env.MARTIN_INTERNAL_URL
  ...
  if (env.TILE_DELIVERY_MODE === 'worker' && checks.tileWorker.status === 'error') {
    healthy = false
  }

  // Valhalla (routing)
  const valhalla = await probeValhallaReadiness(env.VALHALLA_INTERNAL_URL)
  checks.valhalla = {
...
async function checkTileWorkerHealth(): Promise<HealthCheck> {
  ...
  if (!env.TILE_WORKER_INTERNAL_URL) {
    return {
      status: 'error',
      error: 'TILE_WORKER_INTERNAL_URL is required when TILE_DELIVERY_MODE=worker',
    }
  }
  ...
  try {
    const controller = new AbortController()
    const timeout = setTimeout(() => controller.abort(), 3000)
    const res = await fetch(`${env.TILE_WORKER_INTERNAL_URL.replace(/\/$/, '')}/health`, {
...
export async function checkStorageHealth(start = Date.now()): Promise<HealthCheck> {
...
function isDiagnosticsRequestAllowed(headers: Headers) {
  return process.env.NODE_ENV !== 'production' || isInternalRequestAuthorized(headers)
}
