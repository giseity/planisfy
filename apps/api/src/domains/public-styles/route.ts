...
import { db, accounts, stylePublications, styles, styleVersions } from '@planisfy/database'
...
import {
  ...
  styleCacheControl,
  styleEtag,
} from '../resources/public-style-contract'
import { parsePublishedSpriteMetadata, type SpriteVariant } from '../styles/style-sprites'
...
publicStylesRoute.get('/styles/v1/:owner/:handle', async (c) => {
  ...
  const { handle, version, invalidVersion } = parseStyleHandleVersion(c.req.param('handle'))
  ...
  const [style] = await db
    ...
    .from(styles)
    .where(and(eq(styles.ownerId, account.id), eq(styles.handle, handle), isNull(styles.deletedAt)))
    .limit(1)
  ...
  return c.json(
    publishedStyleJson({
      ...
      spriteBaseUrl: sprite ? spriteBaseUrl(c, owner, c.req.param('handle')) : null,
    })
  )
})
...
publicStylesRoute.get('/styles/v1/:owner/:handle/sprite*', async (c) => {
  ...
  const { handle, version, invalidVersion } = parseStyleHandleVersion(c.req.param('handle'))
  if (invalidVersion) {
    return c.json({ error: { code: 'VALIDATION_ERROR', message: 'Invalid style version' } }, 400)
  }
...
async function resolvePublishedVersion(styleId: string) {
  const [publication] = await db
    ...
    .where(and(eq(stylePublications.styleId, styleId), eq(stylePublications.alias, 'latest')))
    .limit(1)
...
async function resolveExplicitVersion(styleId: string, version: number) {
  const [publication] = await db
    ...
    .from(stylePublications)
    .where(and(eq(stylePublications.styleId, styleId), eq(stylePublications.alias, `v${version}`)))
    .limit(1)
...
function spriteBaseUrl(_c: { req: { url: string } }, owner: string, handle: string) {
  return spriteBaseUrlFromApiBase(env.NEXT_PUBLIC_API_URL, owner, handle)
}

export function spriteBaseUrlFromApiBase(apiBase: string, owner: string, handle: string) {
