...
import {
  billingRoute,
  DODO_WEBHOOK_BODY_LIMIT_BYTES,
  dodoWebhookBodyLimit,
...
test('Dodo webhook bodies are rejected before the handler buffers oversized content', async () => {
  let bodyReadCompleted = false
  const app = new Hono()
  app.post('/webhooks/dodo', dodoWebhookBodyLimit, async (c) => {
    const body = await c.req.text()
    bodyReadCompleted = true
    return c.text(body)
  })

  const response = await app.request('/webhooks/dodo', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: 'x'.repeat(DODO_WEBHOOK_BODY_LIMIT_BYTES + 1),
  })
  assert.equal(response.status, 413)
  assert.equal(bodyReadCompleted, false)
})
