...
export function scopesToPermissions(scopes: readonly ApiKeyScope[]): ApiKeyPermissions {
...
export function metadataWithAllowedDomains(allowedDomains: readonly string[]): ApiKeyMetadata {
...
export type ApiKeyPolicyDecision =
  ...
  | {
      allowed: false
      status: 403
      code: 'DOMAIN_NOT_ALLOWED' | 'SCOPE_DENIED'
      message: string
    }

export function evaluateApiKeyPolicy(params: {
  permissions: unknown
  metadata: unknown
  path: string
  origin?: string
}): ApiKeyPolicyDecision {
  const domains = metadataAllowedDomains(params.metadata)
  if (domains.length > 0 && !isRequestOriginAllowed(params.origin, domains)) {
    return {
      allowed: false,
      status: 403,
  ...
  const scopes = permissionsToScopes(params.permissions)
  const requiredScope = requiredScopeForPath(params.path)
...
function normalizeAllowedDomain(value: string): string | null {
  ...
  const host =
    trimmed.startsWith('http://') || trimmed.startsWith('https://') ? hostFromUrl(trimmed) : trimmed
  if (!host) return null
...
function routeCoordinateCount(path: string): number | null {
  const match = path.match(/^\/(?:directions|matching|matrix|optimized-trips)\/v1\/[^/]+\/([^/?]+)/)
