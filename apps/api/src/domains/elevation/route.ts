...
const ELEVATION_INTERNAL_URL = env.ELEVATION_INTERNAL_URL
...
elevationRoute.get('/elevation/v1/:coords', async (c) => {
    ...
    return c.json(
      {
        error: {
          code: 'BAD_REQUEST',
          message: 'Invalid coordinates. Format: lon,lat or lon1,lat1;lon2,lat2',
        },
      },
      400
    )
  ...
  if (
    points.some(
      (p) => p.longitude! < -180 || p.longitude! > 180 || p.latitude! < -90 || p.latitude! > 90
    )
  ) {
    return c.json(
      {
  ...
  if (points.length > 100) {
    return c.json(
      { error: { code: 'BAD_REQUEST', message: 'Maximum 100 points per request' } },
      400
    )
  }

  try {
    const res = await fetch(`${ELEVATION_INTERNAL_URL}/lookup`, {
      method: 'POST',
    ...
    return c.json(
      { error: { code: 'INTERNAL_ERROR', message: 'Elevation service unavailable' } },
      503
    )
  }
})
...
elevationRoute.get('/elevation/v1/along/:coords', async (c) => {
    ...
    return c.json(
      {
        error: {
          code: 'BAD_REQUEST',
          message: 'At least 2 coordinates required for elevation profile',
        },
      },
      400
    )
  ...
  try {
    ...
    const profile: ProfilePoint[] = (data.results || []).map((r: ElevationResult, i: number) => {
      if (i > 0) {
        cumulativeDistance += haversine(
          interpolated[i - 1]!.latitude,
          interpolated[i - 1]!.longitude,
          interpolated[i]!.latitude,
          interpolated[i]!.longitude
        )
      }
      return {
    ...
    if (elevations.length === 0) {
      return c.json(
        { error: { code: 'NO_ELEVATION_DATA', message: 'No elevation data found for this route' } },
        404
      )
    }
  ...
  for (let s = 0; s < numSamples; s++) {
    ...
    for (let i = 0; i < segmentDistances.length; i++) {
      if (accumulated + segmentDistances[i]! >= targetDist || i === segmentDistances.length - 1) {
        const ratio =
          segmentDistances[i]! > 0 ? (targetDist - accumulated) / segmentDistances[i]! : 0
