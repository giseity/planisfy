...
type PlatformCapabilityStatus = 'configured' | 'degraded' | 'unavailable' | 'hidden'
...
async function buildPreflightChecks(
  ...
  return [
    ...
    check({
      ...
      action: 'Set SOURCE_CREDENTIAL_ENCRYPTION_KEY to a base64: 32-byte key in production.',
    }),
    check({
      id: 'api-url',
      group: 'Routing',
    ...
    check({
      ...
      severity: 'recommended',
      ok: Boolean(
        process.env.DUCKDB_PATH && process.env.TIPPECANOE_PATH && process.env.OGR2OGR_PATH
      ),
      warnWhenMissing: true,
      message:
        process.env.DUCKDB_PATH && process.env.TIPPECANOE_PATH && process.env.OGR2OGR_PATH
    ...
    check({
      ...
      severity: managed ? 'required' : 'optional',
      ok: Boolean(
        env.DODO_PAYMENTS_API_KEY && env.DODO_PAYMENTS_WEBHOOK_SECRET && starterProductConfigured
      ),
      warnWhenMissing: !managed,
      message:
        env.DODO_PAYMENTS_API_KEY && env.DODO_PAYMENTS_WEBHOOK_SECRET && starterProductConfigured
...
async function buildUpgradeReadinessChecks(storage: PreflightCheck): Promise<PreflightCheck[]> {
  const manifestPath = process.env.PLANISFY_RELEASE_MANIFEST
  const manifestCheck = await upgradeManifestCheck(manifestPath)
  const manifest =
    manifestCheck.manifest && manifestCheck.status === 'pass' ? manifestCheck.manifest : null
  ...
  return [
    ...
    check({
      ...
      action: 'Set PLANISFY_RELEASE_MANIFEST to a pinned release manifest before upgrading.',
    ...
    check({
      ...
      ok: false,
      warnWhenMissing: true,
      message: 'Worker heartbeat is verified by /health/detailed when Redis is reachable.',
      action:
  ...
  if (!path) {
    return {
      ...
      check: check({
        ...
        action: 'Set PLANISFY_RELEASE_MANIFEST to a pinned release manifest before upgrade.',
      }),
    }
  }
...
async function backupScriptCheck() {
  ...
  return check({
    ...
    action: 'Mount scripts/self-host-backup.sh or run upgrades through the self-host supervisor.',
    value: backupScript,
  })
}
...
async function buildProductLoopChecks(
  ...
  const martinSourcesPath = process.env.MARTIN_SOURCES_PATH ?? join(storagePath, 'martin-sources')
  ...
  return [
    await pathAccessCheck({
      ...
      action: 'Run scripts/self-host-setup.sh to create local storage directories.',
    }),
...
async function styleFixtureCheck(storagePath: string): Promise<PreflightCheck> {
  ...
  return check({
    ...
    action: 'Run scripts/self-host-setup.sh to seed the local demo style fixtures.',
...
function buildCapabilityStates(
  ...
  return getDeploymentPolicy(deploymentMode).capabilities.map((capability) => {
    ...
    const primary =
      mapped.find((item) => item.status === 'fail' && item.severity === 'required') ??
    ...
    return {
      ...
      action: status === 'configured' ? undefined : (primary.action ?? capability.description),
      value: primary.value,
    }
  })
}

function capabilityChecks(id: CapabilityId, checks: PreflightCheck[]): PreflightCheck[] {
...
function fallbackCapabilityState(
  ...
  if (capability.id === 'selfHostSupervisor') {
    const configured = Boolean(process.env.SUPERVISOR_URL && process.env.SUPERVISOR_TOKEN)
    return {
      ...capability,
...
async function storageCheck(deploymentMode = activeDeploymentMode()): Promise<PreflightCheck> {
  ...
  return check({
    ...
    action: 'Set R2 bucket, endpoint or account ID, access key, secret, and public URL.',
