...
test('setup preflight reports self-host product loop fixture readiness', async () => {
  ...
  await writeFile(join(storage, 'styles', 'planisfy-streets-light-v1.json'), '{}')
  await writeFile(join(storage, 'styles', 'planisfy-streets-dark-v1.json'), '{}')
  ...
  try {
    const response = await withMockValhalla(async () => app.request('/setup/preflight'))
    const body = (await response.json()) as {
      data?: {
    ...
    assert.equal(checks.get('upgrade-release-manifest')?.group, 'Upgrade readiness')
  } finally {
...
test('setup preflight surfaces actionable self-host storage failures', async () => {
  ...
  try {
    ...
    assert.equal(capabilities.get('localStorage')?.value, join(storage, 'uploads'))
  } finally {
    if (previousStorage === undefined) {
      delete process.env.LOCAL_STORAGE_PATH
    } else {
...
test('setup preflight reports blocking managed readiness without R2, Dodo, or ZeptoMail', async () => {
  const previousMode = process.env.DEPLOYMENT_MODE
  const previousProvider = process.env.STORAGE_PROVIDER
...
async function withMockValhalla<T>(fn: () => Promise<T>, options: { routeOk?: boolean } = {}) {
  ...
  globalThis.fetch = (async (input, init) => {
    ...
    if (url.endsWith('/route')) {
      return options.routeOk === false
        ? Response.json({ error: 'No suitable edges near location' }, { status: 400 })
