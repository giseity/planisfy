...
import { accounts, db, storageObjects, tilesetVersions, tilesets } from '@planisfy/database'
import { getStorage, type StorageProvider, type StorageProviderInfo } from '@planisfy/storage'
import { and, eq, isNotNull, isNull } from 'drizzle-orm'
import Protobuf from 'pbf'
import { PMTiles, SharedPromiseCache, TileType, type RangeResponse, type Source } from 'pmtiles'

const pmtilesCache = new SharedPromiseCache(200)

export type ResolvedTileset = NonNullable<Awaited<ReturnType<typeof resolveTileset>>>
...
export async function resolveTileset(
  ...
  const [owner] = await db
    ...
    .where(
      and(
        eq(accounts.handle, ownerHandle),
  ...
  const [tileset] = await db
    ...
    .from(tilesets)
    .where(
      and(eq(tilesets.accountId, owner.id), eq(tilesets.handle, handle), isNull(tilesets.deletedAt))
    )
    .limit(1)
...
export async function queryResolvedTileset(
  ...
  try {
    ...
    const tile = await archive.archive.getZxy(tileCoords.z, tileCoords.x, tileCoords.y)
...
export function parsePublicTilesetSlug(slug: string) {
  const match = slug.match(/^([a-z0-9][a-z0-9_-]*)\.([a-z0-9][a-z0-9_-]*)(?:@([1-9]\d*))?$/)
...
export function parseStableTileJsonPath(pathname: string) {
  const match = pathname.match(/\/tiles\/v1\/([^/]+)\/([^/@]+)(?:@([1-9]\d*))?\.json$/)
  if (!match) return null
...
export function parseVersionedTileJsonPath(pathname: string) {
  const match = pathname.match(/\/tiles\/v1\/([^/]+)\/([^/]+)\/versions\/([1-9]\d*)\.json$/)
  if (!match) return null
...
export function parseTileQueryOptions(
  ...
  if (
    (parsedZ !== undefined && (!Number.isInteger(parsedZ) || parsedZ < 0 || parsedZ > 26)) ||
...
): { ok: true; archive: PMTiles } | Extract<TileReadResult | TileQueryResult, { ok: false }> {
  const storageInfo = storage.getInfo()
...
function queryVectorTile(params: {
  ...
  for (const layerName of Object.keys(vectorTile.layers)) {
    ...
    for (let index = 0; index < layer.length; index += 1) {
      ...
      if (
        !geojson.geometry ||
        !geometryNearPoint(geojson.geometry, params.lon, params.lat, params.options.radius)
      ) {
        continue
      }
      ...
      features.push({
        ...
        geometry:
          params.options.geometry === 'full'
            ? geojson.geometry
            : representativePointGeometry(geojson.geometry, params.lon, params.lat),
      })
...
function geometryNearPoint(
  ...
  return minCoordinateDistanceMeters(geometry.coordinates, lon, lat) <= radiusMeters
}
...
function representativePointGeometry(
  ...
  return {
    type: 'Point',
    coordinates: nearestCoordinate(geometry.coordinates, lon, lat) ?? [lon, lat],
  }
}
...
function minCoordinateDistanceMeters(coordinates: unknown, lon: number, lat: number) {
