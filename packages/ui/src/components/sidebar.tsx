...
import { Sheet, SheetContent, SheetTitle, SheetTrigger } from '@planisfy/ui/components/sheet'
...
function SidebarProvider({
  ...
  const [collapsedState, setCollapsedState] = React.useState(defaultCollapsed ?? !defaultOpen)
  const setCollapsed = React.useCallback<React.Dispatch<React.SetStateAction<boolean>>>((value) => {
  ...
  const value = React.useMemo(
    () => ({ collapsed: collapsedState, setCollapsed, toggleCollapsed }),
    [collapsedState, setCollapsed, toggleCollapsed]
  )
  ...
  return (
    <aside
      ...
      className={cn(
        ...
        variant === 'inset' ? 'h-full py-1' : 'min-h-svh border-r',
...
function SidebarHeader({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'flex min-h-14 flex-col gap-1.5 px-1 py-1 pt-2 group-data-[collapsible=icon]/sidebar:items-center',
        className
      )}
      {...props}
    />
...
function SidebarContent({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'flex min-h-0 flex-1 flex-col gap-4 overflow-x-hidden overflow-y-auto p-2',
        className
      )}
      {...props}
    />
...
function SidebarFooter({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'px-1 py-1 group-data-[collapsible=icon]/sidebar:flex group-data-[collapsible=icon]/sidebar:justify-center',
        className
      )}
      {...props}
    />
...
function SidebarInset({ className, ...props }: React.ComponentProps<'main'>) {
  return (
    <main
      ...
      className={cn(
        'flex min-h-0 min-w-0 flex-1 flex-col bg-background md:m-2 md:overflow-hidden md:rounded-xl',
        className
      )}
      {...props}
    />
