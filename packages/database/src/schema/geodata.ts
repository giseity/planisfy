...
import { savedRegions, sourceConnections, styles, styleVersions } from './resources'
import {
...
export const datasetVersions = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex('dataset_versions_dataset_version_unique').on(table.datasetId, table.version),
  ]
)
...
export const rootAgentRegistrationTokens = pgTable(
  'root_agent_registration_tokens',
  {
    ...
    createdWorkerNodeId: uuid('created_worker_node_id').references(() => workerNodes.id, {
      onDelete: 'set null',
    }),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  },
  (table) => [
    ...
    uniqueIndex('root_agent_registration_tokens_hash_unique').on(table.tokenHash),
  ]
)
...
export const routingGraphBuilds = pgTable(
  'routing_graph_builds',
  {
    ...
    elevationMode: varchar('elevation_mode', { length: 64 }).notNull().default('none'),
...
export const stylePublications = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex('style_publications_style_alias_unique').on(table.styleId, table.alias),
  ]
)
...
export const eventOutbox = pgTable(
  'event_outbox',
  {
    ...
    processAt: timestamp('process_at', { withTimezone: true }).notNull().defaultNow(),
...
export const spriteAssets = pgTable(
  'sprite_assets',
  {
    ...
    rasterStorageObjectId: uuid('raster_storage_object_id').references(() => storageObjects.id, {
      onDelete: 'restrict',
    }),
  ...
  (table) => [
    ...
    index('sprite_assets_raster_storage_object_idx').on(table.rasterStorageObjectId),
...
export const geocodingBuilds = pgTable(
  'geocoding_builds',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    accountId: uuid('account_id')
    ...
    peliasDockerCommit: varchar('pelias_docker_commit', { length: 64 }),
    indexName: varchar('index_name', { length: 128 }).notNull().default('pelias'),
    config: jsonb('config').notNull().default({}),
    output: jsonb('output').notNull().default({}),
    errorCode: varchar('error_code', { length: 128 }),
    errorMessage: text('error_message'),
  ...
  (table) => [
    index('geocoding_builds_account_idx').on(table.accountId),
    index('geocoding_builds_status_idx').on(table.status),
    index('geocoding_builds_worker_idx').on(table.workerNodeId),
  ]
)

export const geocodingBuildLogs = pgTable(
  'geocoding_build_logs',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    buildId: uuid('build_id')
      .notNull()
  ...
  (table) => [index('geocoding_build_logs_build_idx').on(table.buildId)]
)

export const geocodingArtifacts = pgTable(
  'geocoding_artifacts',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    accountId: uuid('account_id')
    ...
    snapshotName: varchar('snapshot_name', { length: 128 }).notNull(),
    snapshotRepository: varchar('snapshot_repository', { length: 128 })
      .notNull()
      .default('planisfy'),
    manifest: jsonb('manifest').notNull().default({}),
    errorMessage: text('error_message'),
...
export const geocodingReleases = pgTable(
  'geocoding_releases',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    accountId: uuid('account_id')
    ...
    activationStatus: varchar('activation_status', { length: 32 }).notNull().default('inactive'),
    isPrimary: boolean('is_primary').notNull().default(false),
    buildId: uuid('build_id').references(() => geocodingBuilds.id, {
      onDelete: 'set null',
    }),
    artifactId: uuid('artifact_id').references(() => geocodingArtifacts.id, {
      onDelete: 'set null',
    }),
    sourceDataVersions: jsonb('source_data_versions').notNull().default({}),
    manifest: jsonb('manifest').notNull().default({}),
  ...
  (table) => [
    index('geocoding_releases_account_idx').on(table.accountId),
    index('geocoding_releases_build_idx').on(table.buildId),
    uniqueIndex('geocoding_releases_name_version_unique').on(
      table.accountId,
      table.name,
      table.version
    ),
    uniqueIndex('geocoding_releases_primary_unique')
      .on(table.accountId)
...
export const usageRollups = pgTable(
  ...
  (table) => [index('usage_rollups_account_period_idx').on(table.accountId, table.periodStart)]
)
