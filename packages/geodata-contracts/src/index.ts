...
export const WORKER_GEODATA_HEARTBEAT_KEY = 'planisfy:worker-geodata:heartbeat'
export const WORKER_GEODATA_HEARTBEAT_STALE_MS = 60_000

export const MANAGED_PELIAS_PROFILE = 'planet_geocoder' as const
export const MANAGED_PELIAS_PROFILE_VERSION = 1 as const
export const MANAGED_PELIAS_PROFILE_IMPORTERS = [
  'whosonfirst',
  'geonames',
  'openaddresses',
  'openstreetmap',
...
const SOURCE_FORMATS = ['geojson', 'csv', 'shapefile', 'pmtiles', 'mbtiles'] as const
...
export function parseSourceProcessingJobInput(input: unknown): SourceProcessingJobInput {
  if (!isRecord(input)) {
  ...
  if (typeof input.uploadId !== 'string' && typeof input.datasetVersionId !== 'string') {
...
export function isTilesetBuildFormat(value: unknown): value is TilesetBuildFormat {
